...
fn calculate_visible(grid: Vec<Vec<usize>>) -> usize {
    ...
    for (i, line) in grid.iter().enumerate() {
        for (j, value) in line.iter().enumerate() {
            // top
            let t_visible =
                check_directions(grid.clone(), *value, i, j, |x, y| (Some(x + 1), Some(y)));

            // bottom
            let b_visible = if i == 0 {
                true
            } else {
                check_directions(grid.clone(), *value, i, j, |x, y| {
                    (x.checked_sub(1), Some(y))
                })
            };

            // left
            let l_visible = check_directions(grid.clone(), *value, i, j, |x, y| {
            ...
            let r_visible = if j == 0 {
                ...
                check_directions(grid.clone(), *value, i, j, |x, y| (Some(x), Some(y + 1)))
            };

            if t_visible || b_visible || l_visible || r_visible {
                if i != 0 && j != 0 && i != 4 && j != 4 {
                    println!("visible!: {i} {j} - {value}, t: {t_visible}\t b: {b_visible}\t  l: {l_visible}\t r: {r_visible}");
                }
